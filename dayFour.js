...
function solveDay4B() {
    let valid = 0;
    let keys = ["pid", "iyr", "hgt", "hcl", "ecl", "eyr", "byr"]
    let isValid;

    for (let i = 0; i < inData.length; i++) {
        isValid = true;
        let obj = parseData(inData[i]);
        //console.log(obj)
        for (let j = 0; j < keys.length; j++) {

            if (obj.hasOwnProperty(keys[j])) {
                if (!validatorObject[keys[j]](obj[keys[j]])) {
                    isValid = false;
                    break;
                }
            } else {
...
let validatorObject = {
    ...
    iyr: function (iyr) {
        let numIYR = Number(iyr);
        return ((numIYR >= 2010) && (numIYR <= 2020));
    },
    hgt: function (hgt) {
        let indexIN = hgt.indexOf('in')
        let indexCM = hgt.indexOf('cm')
        let height;
        if (indexIN > -1) {
            // imperial detected
            height = Number(hgt.substr(0, indexIN))
            return (height >= 59 && height <= 76);
        } else if (indexCM > -1) {
            // metric detected
            height = Number(hgt.substr(0, indexCM))
    ...
    hcl: function (hcl) {
        ...
        if (!(hcl[0] === '#')) {
            console.log("leading hashtag failed")
            return false;
        }

        for (let i = 1; i < hcl.length; i++) {
            if (!(((hcl.charCodeAt(i) >= 97) && (hcl.charCodeAt(i) <= 102)) || ((hcl.charCodeAt(i) >= 48) && (hcl.charCodeAt(i) <= 57)))) {
                console.log("char fail: " + hcl.charCodeAt(i))
                return false;
            }
        }
        return true;
    ...
    eyr: function (eyr) {
        let numEYR = Number(eyr);
        return ((numEYR >= 2020) && (numEYR <= 2030));
    },
    byr: function (byr) {
        let numBYR = Number(byr);
        return ((numBYR >= 1920) && (numBYR <= 2002));
    }
}
...
function validateObject(obj) {
    ...
    keys.forEach(function (key) {
        // kolla om keun finns?
        // i så fall, är den valid?
    });

}

function solveDay4A() {
    let validPassports = 0, foundCID;
    for (let i = 0; i < inData.length; i++) {
        foundCID = false;
        if (countOccurences("cid", inData[i])) {
            foundCID = true;
        }
        if (countOccurences(":", inData[i]) === 8) {
            validPassports++;
        }
        if (countOccurences(":", inData[i]) === 7 && !foundCID) {
            validPassports++;
        }
    }
    return validPassports
}
console.log(solveDay4B())

function countOccurences(letter, string) {
    let count = 0, i = 0, pos;
    while (i < string.length) {
        pos = string.indexOf(letter, i)
        if (pos > -1) {
            count++;
            i = pos + 1;
        } else {
