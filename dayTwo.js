...
function solveDay2B() {
    let foundA, foundB, validPasswords = 0;
    // parseData
    // a är en position (från index 1) i password
    //           - " - 
    // exakt 1 av positionerna a och b måste innehålla letter i password
    // MEN INTE BÅDA!!!
    for (let i = 0; i < inData.length; i++) {
        let obj = parseData(inData[i]); // {a: 12, b: 34, letter 'q', password: 'sldkjf}
        foundA = (obj.password[obj.a - 1] === obj.letter) ? true : false;
        foundB = (obj.password[obj.b - 1] === obj.letter) ? true : false;

        validPasswords += ((foundA || foundB) && !(foundA && foundB)) ? 1 : 0;
    }
...
function parseData(str) {
    // "2-3 g: gggl"
    // splitta 2 ggr, först på mellanrum
    let terms = str.split(' '); // ["19-20", "q:", "kjwelkj"]
    let range = terms[0].split('-'); // ["19", "20"]
    let myObj = {};
    myObj.a = Number(range[0]);
    myObj.b = Number(range[1]);
    myObj.letter = terms[1].substr(0, 1);
    myObj.password = terms[2];
    return myObj;
}
...
function solveDay2A() {
    ...
    for (let i = 0; i < inData.length; i++) {
        let object = parseData(inData[i]);
        let count = countOccurences(object.letter, object.password)
...
console.log(solveDay2B());

function oldParseData(str) {
    // "2-3 g: gggl"
    // splitta 2 ggr, först på mellanrum
    let terms = str.split(' '); // ["19-20", "q:", "kjwelkj"]
    let range = terms[0].split('-'); // ["19", "20"]
    let myObj = {};
    myObj.min = Number(range[0]);
    myObj.max = Number(range[1]);
    myObj.letter = terms[1].substr(0, 1);
    myObj.password = terms[2];
    return myObj;
}

// skriv en funktion som räknar antalet förekomster av ett tecken i en sträng
